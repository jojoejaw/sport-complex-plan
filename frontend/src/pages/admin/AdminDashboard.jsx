      ...
      <div className={`h-full min-h-0 rounded-[22px] border border-[#e4ebe6] bg-[#f7f9fc] shadow-[0_12px_34px_rgba(20,50,30,0.08)] max-lg:h-auto max-lg:overflow-visible ${activeSection === 'dashboard' ? 'overflow-hidden' : 'overflow-y-auto [scrollbar-gutter:stable]'}`}>
        <main className="p-2.5 lg:p-3" onClick={(event) => {
          ...
          <div className="mt-3 grid gap-3 xl:grid-cols-[1.35fr_0.95fr]">
            <Panel title={`รายได้รับ${periodText} (อิงวันที่ชำระสำเร็จ/อนุมัติ)`} className="min-h-[300px]"><RevenueChart values={dashboard.chartValues} showPointLabels={dashboardPeriod === 'total'} /></Panel>
            <Panel title={`สัดส่วนการจองที่ยังใช้งานตามประเภทกีฬา (${periodText})`} className="min-h-[300px]"><div className="flex h-[220px] items-center justify-center gap-8 max-sm:flex-col"><Donut parts={sportParts} /><div className="w-full max-w-[210px] space-y-3">{sportParts.map((item) => <p key={item.label} className="grid grid-cols-[10px_1fr_auto] items-center gap-2 text-xs"><i className="h-2.5 w-2.5 rounded-full" style={{ background: item.color }} /><span className="truncate">{item.label}</span><b>{Math.round((item.value / Math.max(1, dashboard.activePeriodBookings.length)) * 100)}% ({item.value})</b></p>)}</div></div></Panel>
          </div>
